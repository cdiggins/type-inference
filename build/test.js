...
function catTypeToString(t) {
    if (t instanceof type_system_1.TypeVariable)
        return "'" + t.name;
    else if (t instanceof type_system_1.TypeArray)
        return "(" + t.types.map(catTypeToString).join(" ") + ")";
    else
        return t.toString();
}
exports.catTypeToString = catTypeToString;
function compareTypes(t1, t2) {
    {
        var r1 = type_system_1.normalizeVarNames(t1).toString();
        var r2 = type_system_1.normalizeVarNames(t2).toString();
        if (r1 != r2) {
            throw new Error("Types are not the same when normalized: " + r1 + " and " + r2);
        }
    }
    {
        var r1 = type_system_1.alphabetizeVarNames(t1).toString();
        var r2 = type_system_1.alphabetizeVarNames(t2).toString();
        if (r1 != r2) {
...
function testCatTermsCommon() {
    ...
    for (var _i = 0, catTests_1 = catTests; _i < catTests_1.length; _i++) {
        ...
        runTest("Cat Inference Test " + i++, function (x) { return type_system_1.normalizeVarNames(cat_types_1.inferCatType(x)).toString(); }, xs[0], xs[1]);
    }
}
function issue1InCat() {
    var t = cat_types_1.inferCatType("");
    t = type_system_1.alphabetizeVarNames(t);
    console.log(t.toString());
...
function testCombinator(name, term, exp) {
    try {
        ...
        console.log("Inferred type: " + t);
        console.log("Simplified type: " + type_system_1.typeToString(t));
        if (exp != null)
...
function testCatTermsWithVariance() {
    ...
    for (var _i = 0, catTestStrings_1 = catTestStrings; _i < catTestStrings_1.length; _i++) {
        ...
        runTest("Test Cat Inference with Variance " + i++, cat_types_1.inferCatType, ct);
    }
}
testCatTermsCommon();
